...
const STORAGE_KEY = "support_conversation_id_v1";
...
export default function ChatWidget() {
  ...
  async function fetchGeo() {
    try {
      ...
      // silently fail, we just won't have location
      return {};
    }
  }
  ...
  const suggestions = [
    "I want EU residency – where do I start?",
    "Help me compare 2–3 citizenship by investment options.",
    "What’s the best program for a family of 4 with $250k+ budget?",
  ];

  return (
    <>
      {/* Bubble button – bottom-left so it doesn't clash with your phone button */}
      <button
        onClick={() => setOpen((o) => !o)}
        className="
          fixed 
          ...
          <div className="px-4 py-3 border-b bg-slate-50 text-[11px] leading-relaxed space-y-1">
            <p className="text-slate-700">
              ...
              <span className="font-semibold">
                budget, family size and preferred region
              </span>{" "}
              and we’ll suggest suitable residency or citizenship routes.
            </p>
            <p className="text-slate-600">
              Typical reply:{" "}
              <span className="font-semibold">under 1 hour</span> on business
              days.
            </p>
          </div>

          {/* Name/email prompt (only before first message) */}
          {!conversationId && (
            <div className="px-4 py-3 border-b space-y-2 text-[11px]">
              <p className="text-slate-700">
                Add your details so an advisor can follow up:
              </p>
              <input
